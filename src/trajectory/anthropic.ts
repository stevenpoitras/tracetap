...
export class AnthropicAdapter implements AgentAdapter {
  ...
  matches(pair: RawPair): boolean {
    ...
    // sizing probe, not a model call: no completion, no usage, no cost. The
    // default proxy matcher is `/v1/messages`, which prefix-matches it, so it
    // reaches this adapter — and indexed as a turn it manufactures phantom
    // sessions with 0 turns, 0 tokens and $0, plus a matching hole in
    // usage_events. Match on the URL, since the body cannot tell the two apart.
