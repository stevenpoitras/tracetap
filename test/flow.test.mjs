...
test("a request with unknown ts (0) does not flush the hook lane early", () => {
  // RequestRow.ts is 0 when unknown; that must read as "do not flush yet",
  // not as the drain-everything sentinel — otherwise every hook (here a Stop
  // that fired an hour later) is chained before the first step.
  const flow = deriveFlow({
    steps: [
      {
        stepIndex: 1,
  ...
  assert.ok(ids.indexOf("hook-7") > ids.indexOf("api-0"), "hook is not chained before the API call");
  assert.equal(ids[ids.length - 1], "hook-7", "unknown ts defers the hook to the trailing flush");
});
